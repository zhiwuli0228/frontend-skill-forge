...
The stitch run is executed by an agent following the `frontend-registry-stitch` Skill (located in `frontend-skill-template/frontend-registry-stitch/`), consuming this protocol as its reference. The agent is responsible for running the MCP session, parsing the code-side index, executing the matching algorithm, and writing the registry. Browser element snapshots may be provided by `frontend-e2e-explorer` (located in `frontend-skill-template/frontend-e2e-explorer/`, as a side effect of its Step 3 route exploration) or captured directly by this Skill.
